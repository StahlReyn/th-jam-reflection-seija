...
func _physics_process(delta: float) -> void:
	...
	match state:
		State.STATIC:
			...
			laser_collision.shape.size.y = MathUtils.lerp_smooth(laser_collision.shape.size.y, target_size.y, lerp_speed, delta)
			var target_scale = target_size.y / base_size.x
			main_sprite.scale.x = MathUtils.lerp_smooth(main_sprite.scale.x, target_scale, lerp_speed, delta)
		State.END:		
			laser_collision.shape.size.y = MathUtils.lerp_smooth(laser_collision.shape.size.y, 0.0, lerp_speed, delta)
			main_sprite.scale.x = MathUtils.lerp_smooth(main_sprite.scale.x, 0.0, lerp_speed, delta)
